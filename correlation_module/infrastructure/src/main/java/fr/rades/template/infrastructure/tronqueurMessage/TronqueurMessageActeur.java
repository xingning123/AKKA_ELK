...
public class TronqueurMessageActeur extends AbstractActor implements TraiteurString, TraiteurDate, SeparateurStringEnDocuments {
  ...
  private final Gson gson = new Gson();
    ...
    private MessageTronqueurTraiteur jsonEnMessagePourTraiteur(JsonObject documentJson) throws ParseException  {
        System.out.println("the first id is :"+documentJson.get("id_commande"));
        if (documentJson.get("id_commande") == null) {
            System.out.println("error data");
//            throw new ParseException ("缺少'id_commande'字段或字段值为空",72);
            return null;
        }

        else {String id_commande = supprimerBordureMots(documentJson.get("id_commande").getAsString(),'"');
        String timestampString = supprimerBordureMots(documentJson.get("@timestamp").getAsString(),'"');
        ...
        return receiveBuilder()
                .match(MessageRecuperateurTronqueur.class , messageRecuperateurTronqueur -> {
                    ...
                    for (JsonObject jsonObject : documentsJsonSepares) {
                        ...
                        if(messageTronqueurTraiteur == null|| messageTronqueurTraiteur.getId_commande() == null || messageTronqueurTraiteur.getListeInfosEtats() == null){

                            System.out.println("can't tell without id or liste etat !");
